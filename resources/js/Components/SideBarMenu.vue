...
<template>
    <div class="text-slate-300 pt-5 pb-20">
        <div class="flex justify-center">
            <div class="rounded-full flex items-center justify-center bg-info text-slate-300 w-24 h-24 text-4xl uppercase">
                {{
        ...
        <ul class="space-y-2 my-4">
            ...
            <li class="text-white rounded-lg hover:bg-primary dark:hover:bg-primary" v-bind:class="
                route().current('dashboard')
                    ? 'bg-primary'
                    : 'bg-slate-700/40 dark:bg-slate-800/40'
            ">
                <Link :href="route('profile.documents')" class="flex items-center py-2 px-4">
                <HomeIcon class="w-6 h-5" />
                <span class="ml-3">{{ lang().label.documents }}</span>
                </Link>
            </li>


        <li v-show="can(['view-user'])" class="py-2">
            <p>{{ lang().label.data }}</p>
        </li>
            <li v-show="can(['view-user'])" class="text-white rounded-lg hover:bg-primary dark:hover:bg-primary"
                v-bind:class="
        ...
        <li v-show="can(['view-role', 'view-permission'])" class="py-2">
            <p>{{ lang().label.access }}</p>
        </li>
        <li v-show="can(['view-role'])" class="text-white rounded-lg hover:bg-primary dark:hover:bg-primary"
                                                        v-bind:class="
        ...
        <!-- <li v-show="can(['view-permission'])" class="text-white rounded-lg hover:bg-primary dark:hover:bg-primary"
                                                        v-bind:class="
