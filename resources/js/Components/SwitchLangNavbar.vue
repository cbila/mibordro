...
<template>
    <div>
        <span v-tooltip="lang == 'tr' ? 'İngilizceye geç' : 'Switch to TR'"
            ...
            <Link v-if="lang == 'tr'" :href="route('setlang', 'en')" class="flex items-center space-x-2">
            <span class="w-5 h-5 fill-current"> EN </span>
            </Link>
            <Link v-if="lang == 'en'" :href="route('setlang', 'tr')" class="flex items-center space-x-2">
            <span class="w-5 h-5 fill-current"> TR </span>
            </Link>
        </span>
    </div>
</template>
