...
import Create from "@/Pages/SGK/Create.vue";
import Edit from "@/Pages/SGK/Edit.vue";
import Delete from "@/Pages/SGK/Delete.vue";
import ForceDelete from "@/Pages/SGK/ForceDelete.vue";
...
watch(
    ...
    debounce(() => {
        ...
        router.get(route("sgk.index"), params, {
            replace: true,
            preserveState: true,
            preserveScroll: true,
        });
...
const restore = (sgk) =>{
    router.patch(route('sgk.restore', sgk));
}
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex justify-end mb-4 text-right">
            ...
            <Edit :show="data.editOpen" @close="data.editOpen = false" :title="props.title" :sgk="data.sgk"/>


            <Delete :show="data.deleteOpen" @close="data.deleteOpen = false" :sgk="data.sgk"/>
            <ForceDelete :show="data.forceDeleteOpen" @close="data.forceDeleteOpen = false" :sgk="data.sgk" :title="props.title" />
            ...
            <div class="space-y-4">
                ...
                <div class="relative bg-white dark:bg-slate-800 shadow sm:rounded-lg">
                    ...
                    <div class="overflow-x-auto scrollbar-table">
                        <table class="min-w-full">
                            ...
                            <tbody class="bg-white">
                                <tr v-for="(sgk, index) in sgks.data" :key="sgk.id">
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                        <div class="flex items-center">
                                            <div class="ml-4">
                                                ...
                                                <div class="text-sm leading-5 text-gray-500">{{ sgk.address }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="whitespace-nowrap py-4 px-2 sm:py-3 border-b border-gray-200">
                                        ...
                                        <div v-show="data.params.trashed" class="rounded-md overflow-hidden">
                                            <SecondaryButton v-show="can(['update-company'])" type="button" @click="
                                                restore (sgk)" 
                    ...
                    <div class="flex justify-between items-center p-2 border-t border-slate-200 dark:border-slate-700">
                        <pagination class="mt-6" :links="props.sgks.links" :filters="data.params" />
                    </div>
                </div>
            </div>
        </div>
